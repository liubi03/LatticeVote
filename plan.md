...
│
├── paper/                       # 论文相关
│   └── A\_Homomorphic\_LWE\_Based\_E-voting\_Scheme.pdf
│
├── code/                        # 所有代码
...
│   │
│   ├── demo/                    # 演示脚本（命令行快速演示）
│   │   └── run\_vote.py          # 运行一个模拟选举的脚本（无Web界面）
│   │
...
| 角色       | 英文名称                            | 职责说明                                                                                                                             |
| :------- | :------------------------------ | :------------------------------------------------------------------------------------------------------------------------------- |
| **注册机构** | Registration Authority ($A\_1$) | - 负责选民身份注册- 为每位合法选民生成签名密钥对 (upk, usk)- 维护并公开合法选民列表 $\mathcal{L}\_{\mathcal{U}}$                                                  |
| **公告板**  | Bulletin Board (BB)             | - 接收并验证选票的合法性（签名、格式、防重放）- 拥有自己的公私钥对 (pk\_BB, sk\_BB)，用于解密选票中的辅助信息 aux- 对验证通过的选票执行同态累加操作- 投票结束后发布最终密文                             |
| **受托人**  | Trustees ($\mathcal{T}$)        | - 一组 $t$ 个受托人，共同持有 LWE 解密私钥- 每个受托人独立生成自己的 LWE 密钥对 (pk\_i, sk\_i) 及主陷门 $R\_i$- 计票阶段，每个受托人提供一份“密文陷门”作为部分解密证明- 任何人可用这些陷门验证并解密最终计票结果 |
| **选民**   | Voter                           | - 持有注册机构颁发的签名密钥 (upk, usk)- 将自己的投票选择编码为二进制比特，使用 LWE 公钥加密- 对加密后的选票签名，并附上辅助信息 aux- 将最终选票提交给公告板                                     |
...
- **公告板 (BB)** 生成自己的密钥对 (pk\_BB, sk\_BB)，公开 pk\_BB。
- **受托人** 各自生成 LWE 密钥对及辅助密钥：
  - 每个受托人 $i$ 生成 LWE 私钥 $s\_i$、主陷门 $R\_i$ 和公钥 $pk\_i$。
  - 生成用于同态运算的辅助密钥 $pk^{(f)}$, $pk^{(m)}$ 及 bootstrapping 密钥 $BK\_1, BK\_2, BK\_3$。
- 系统公开发布所有公钥和参数：$pk\_{BB}$, ${pk\_i}$, $pk^{(f)}$, $pk^{(m)}$, $BK\_1, BK\_2, BK\_3$，以及空选民列表 $\mathcal{L}\_{\mathcal{U}}$。

#### 2. 选民注册 (Register)

- 选民提供身份 $id$ 给注册机构 $A\_1$。
- $A\_1$ 运行签名密钥生成算法 $\mathsf{KeyGenS}$，输出 $(upk\_{id}, usk\_{id})$。
- $A\_1$ 将 $upk\_{id}$ 加入公开选民列表 $\mathcal{L}_{\mathcal{U}}$，并将 $(upk_{id}, usk\_{id})$ 安全返回给选民。
...
- **每个受托人 $i$** 对每个 $C\_j$ 用自己的主陷门 $R\_i$ 生成密文陷门 $\Pi\_{i,j}$ 并公开。
- **任何人** 可以运行 $\mathsf{VerifyTally}$：
  - 验证每个 $\Pi\_{i,j}$ 是否有效（即是否确实是 $C\_j$ 的合法解密陷门）。
  - 若全部有效，使用这些陷门解密 $C\_j$，得到每个候选人的得票数 $n\_j$。
...
- [x] 在线检索论文相关信息和阅读plan.md文件，详细了解协议内容。
- [x] 确定项目的环境，需要配置哪些包，人工配置环境
- [x] 配置好环境后，开始编写代码
  - [x] 从后端开始实现，使用量子密码，实现投票流程
  - [x] 人工调试和测试后端
  - [x] 实现前端可视化
  - [x] 人工调试和测试前端
- [x] 编写readme文件
...
**当前任务**：实现账户系统（包含零知识证明功能）

**核心需求**：
1. 三种角色：管理员（注册机构）、受托人、选民
2. 自主注册 + 管理员核验机制
3. 基于数字签名的选民认证
4. 零知识证明功能（选民身份证明、选票有效性证明、计票正确性证明）
5. **投票过程演示功能**：管理员可以查看已完成项目，点击按钮详细展示加密、解密、同态运算过程

**技术环境**：
- Conda 环境：LatticeVote
- 所有依赖包已安装

**实现计划**：
按照 tasks.md 中的 14 个任务逐步实现
- Task 1: 创建目录结构
- Task 2-4: 核心模块（用户管理、数字签名、零知识证明）
- Task 5-7: 业务模块（认证、项目管理、选票管理）
- Task 8-11: Web 界面
- Task 12: 集成同态加密
- Task 13-14: 测试和文档
...
### 2026-04-11 完成账户系统核心功能

**已完成**：
- ✅ Task 1-12: 所有核心模块和 Web 界面已实现
- ✅ 用户管理模块（注册、登录、审核）
- ✅ 数字签名模块（RSA 密钥对、签名验证）
- ✅ 零知识证明模块（选民身份、选票有效性、计票正确性）
- ✅ 投票项目管理模块（创建、状态管理、受托人指定）
- ✅ 选票管理模块（加密、签名、零知识证明、同态计票）
- ✅ Web 界面（登录、注册、管理员、受托人、选民控制台）
- ✅ **投票过程演示功能**（详细展示加密、解密、同态运算过程）
...
# 运行账户系统 Web 界面
cd d:\大三下\量子和后量子密码\中期展示\code\account_system
